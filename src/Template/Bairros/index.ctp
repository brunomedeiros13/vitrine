<br><br><br>
<div class="publicidade-topo" style="margin-top: 45px;">
...
<div class="bairros index large-9 medium-8 columns content">
    <h3 style="margin-bottom: 40px"><i class="fas fa-building"></i> Bairros de São José</h3>
    <?php
        ...
        <div class="col-4">
            <?php foreach ($letraas as $letraa): ?>
                ...
                <?php foreach ($bairros as $bairro): ?>                                
                    <?php $nome = substr($bairro->nomeBairro, 0, 1); ?>
                    <?php if ($nome == $letraa) { ?>
                        <?php if ($i == 0) { ?>
                            <h5><?= $letraa ?></h5>
                            <?php $i++ ?>                           
                        <?php } ?>
                        <p style="margin-bottom: 0px"><?= $this->Html->link($bairro->nomeBairro, ['controller' => 'empresas', 'action' => 'pesquisar', 'bairro' => $bairro->nomeBairro]) ?></p>
                        <?php $hr++ ?>
                    <?php } ?>
                <?php endforeach; ?>
                <?php if ($hr > 0) { ?>
